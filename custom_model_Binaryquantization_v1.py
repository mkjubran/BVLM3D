...
# -------------------------------
# 8. Get Model Size (for FP32 and INT8)
# -------------------------------
...
def get_binary_model_size(model):
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            # Weights are stored as 1-bit in binary model
            total_weight_bits += param.numel()  # 1 bit per weight
        elif 'bias' in name:
            # Biases are stored as FP32 (4 bytes per bias)
            total_bias_bytes += param.numel() * 4

    # Convert bits to bytes for weights and add bias bytes
...
# -------------------------------
def main():
    data_dir = "../ModelNet40/ModelNet40_2D"
