...
class BinarySimpleCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.features = nn.Sequential(
            BinaryConv2d(3, 16, 3, padding=1), 
            nn.ReLU(inplace=True), 
            nn.MaxPool2d(2),
            BinaryConv2d(16, 32, 3, padding=1), 
            nn.ReLU(inplace=True), 
            nn.MaxPool2d(2),
...
def train_model(model, dataloader, device, save_path, epochs=10, lr=1e-3, model_type="fp32"):
    ...
    for epoch in range(epochs):
        ...
        if model_type == 'fp32':
            ...
            size = get_model_size(model)  # For int8, we'll calculate size after quantization
        ...
        if ((epoch + 1) % 5 == 0) or (((epoch + 1) < 10 )and ((epoch + 1) % 2 == 0)):
            checkpoint_path = f"../saved_models/{model_type}_model_epoch{epoch+1}.pth"
...
def main():
    ...
    parser.add_argument('--model_type', choices=['fp32', 'dynamic_quant', 'int8', 'binary'], required=True,
                       help='Model type: fp32, dynamic_quant, int8, or binary')
    ...
    if args.mode == 'train':
        ...
        if args.model_type == 'fp32':
            ...
            train_model(fp32_model, train_loader, device, fp32_save_path, epochs=args.epochs, model_type="fp32")
